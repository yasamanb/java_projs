...
class Game {
    ...
    /** The undo method, returns game state to previous one. */
    void undo() {
        int current_index = _savedGames.size();
        if (current_index > 0) {
            copyFrom(_savedGames.get(current_index - 1));
            _savedGames.remove(current_index - 1);
        }
    }

    /** Saves a snapshot of the current game in array _savedGames. */
    void savingGame() {
        _savedGames.add(new Game(this));
    }
    ...
    /** The array of savedGames. */
    private ArrayList<Game> _savedGames;
