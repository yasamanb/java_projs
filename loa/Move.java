...
class Move {
     ...
     *  1 <= C <= M, 1 <= R <= M. I modified this method from
    ...
    /** My added method. Returns the compass direction
    associated with a move. */
    Direction getDir() {
        int rdiff = (int) Math.signum((float) (_row1 - _row0));
        int cdiff = (int) Math.signum((float) (_col1 - _col0));
        Direction dir = NOWHERE;
        while (dir != null) {
            if (dir.dc == cdiff && dir.dr == rdiff) {
                break;
            }
            dir = dir.succ();
        }
    ...
    /** The set of all possible Moves, indexed by row and column
    of start, row and column of destination, piece moved and piece
    replaced. */
